...
/* January 7, 1991  added code to store clipping in host coords */
...
int  old_sub_3d_matrix[8];
int  new_sub_3d_matrix[8];
char  overwrite_blinks = 'f';
...
struct a_clipping  the_host_clipping[NUMBER_OF_SUBS];



void  update_a_header(the_data,which_field,which_segment)
...
int  read_subs_matrix(which_sub)
int which_sub;
{
int i,real_sub;
...
void  cap_M_func(sub_count,which_subr) /*insert something*/
	...
	switch(megatek_data[0] & 0x07)
	{
		...
		case 6 : /*insert clipping*/
		{
			...
			else
			{
				...
				the_host_clipping[sub_nums[which_sub]].low_x =
					an_element.arg1;
				the_host_clipping[sub_nums[which_sub]].low_y =
					an_element.arg2;
				the_host_clipping[sub_nums[which_sub]].up_x =
					an_element.arg3;
				the_host_clipping[sub_nums[which_sub]].up_y =
					an_element.arg4;
...
void  cap_J_func(sub_count,which_subr) /*change something*/
int *sub_count,which_subr;
{
int i,j;
float k;
...
#ifdef HAVE_DIAGS
			sprintf(diag_string,"change 3D origin \n");
...
#ifdef HAVE_DIAGS
			sprintf(diag_string,"change Z origin \n");
			my_diag(diag_string);
#endif
