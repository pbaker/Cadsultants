...
int num_ems_pages;
	...
	sprintf(config_text[2],"    Emulation Mode :  D135a");
	...
	while(9)
	{
		...
		switch(selection)
		{
			case 1 : /*set size of listing characters*/
			{
				...
				xor_draw_my_listing(); /* display @ new size */
...
display_main_menu:
		...
		save_again:
				read_menu_string("Enter Save Image File Name",
					name_save_display);
				...
				display_file = fopen(name_save_display,"w+b");
			...
			/*restores original keyboard interrupt vector address*/
				restore_int9();  
