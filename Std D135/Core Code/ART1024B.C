...
int radius,x,y;
{
	if(num_pages != 2)
		...
		lincir(x,y + 768*which_page,radius);
}
...
/*sets version_str[] for displaying version# & date*/
	...
	switch(port_byte)
	{
		case 0x03 :
		{
			sprintf(board_string,"Artist 10E  1024 x 768");
			num_pages = 2;
			break;
		}
		...
		case 0x0f :
		{
			sprintf(board_string,"Artist 10  1024 x 768");
			num_pages = 2;
			break;
		}
		...
		case 0x0c :
		{
			sprintf(board_string,"Artist 10/16  1024 x 768");
			num_pages = 1;
			break;
		}
...
int x1,y1,x2,y2;
{
	if(num_pages == 2)
	{
		linxy(x1,y1 + which_page*768,x2,y2 + which_page*768);
	}
	else
		linxy(x1,y1,x2,y2);
}
...
int the_char,x,y,rotate;
{
	if(num_pages == 2)
	{
		schar(the_char,x + 150,(int)(1.5*y) + which_page*768,rotate);
	}
	else
