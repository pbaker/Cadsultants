...
extern void far command_logger(char far *);
extern char far command_log_string[];

extern int far read_host_buffer(void);
extern void far vt100_pckb(void);
extern void far ugout(int);

extern char far which_terminal;



int far vt100_mess_mon_flag = VT100_MODE;
char far curser_key_mode = CURSER_MODE;
char far keypad_key_mode = NUMERIC_MODE;

char far shift_out_chars[256];
char far shift_flag = SHIFT_IN;

char far escape_push_back = '\0';


char far escape_parameter[22];
char far escape_host_data[100];
char far vt100_attribute = 15;

char far parameters[25][9];


char far escape_error[100];
char far escape_output[50];

int far num_parameters = 0; 

char far escape_buffer[1000];


int far top_of_scroll_region = 0;
int far bottom_of_scroll_region = 24;


int far curser_row_save = 0;
int far curser_col_save = 0;


char far the_error_message[222];



char far is_letter(ch)
int ch;
{
	if((ch >= 'a') && (ch <= 'z'))
		return(TRUE);
...
void far escape_error_handler(a_string)
char far a_string[];
{
...
void far escape_read_more(number)
{
int i,ch;
...
char far is_parameter_character(the_char)
int the_char;
{
...
void far read_escape_parameter(the_string)
char far the_string[];
{
int i;
...
void far erase_screen(which_way)
int which_way;
{
	switch(which_way)
	{
...
void far reset_question_param(which_one)
int which_one;
{
...
void far set_question_param(which_one)
int which_one;
{
	switch(parameters[which_one][1])
	{
...
void far insert_n_chars(number)
int number;
{
...
void far tabulation_clear(which_way)
char which_way;
{
...
void far select_graphic_rendition()
{
int i;
int a_param;
...
void far host_escape(first_char)
int first_char;
{
int save_row,save_col;
