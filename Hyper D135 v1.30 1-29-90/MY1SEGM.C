...
extern void far my_insert_indicator(char,int,int,int);
...
char far inserting_chars = 'f';
int far insert_char_index;
int far x_insert_chars;
int far y_insert_chars;
char far no_more_ems_pages = 'f';




void far out_of_ems_pages()
{
...
int far get_next_free_page(which_type)
int which_type;
{
int i;
...
/*switch to next logical page when needed*/
	...
	if(phy_address_of_insert - phy_address_of_page > 16360L) /*next page*/
	{
		...
		dlint(DL_BRANCH_INTERRUPT + current_logical_page);
		...
		switch(an_element.op_code)  /*decide which pixel command to use*/
		{
			case 0 : /*subroutine call*/
			{
					...
					(which_insert_mode == TO_BOTH))
				{
					pw_dl_init(current_pointer_to_dl,0,
						NULL,0);
					call(&(sub_calling_table[an_element.
						arg1][0]));
			...
			case 14 : /*move 2d absolute*/
			{
				   ...
				   (which_insert_mode == TO_LIST_ONLY))
				{
 					pw_dl_init(current_pointer_to_dl,0,
						NULL,0);
					move3(2*an_element.arg1,
						2*an_element.arg2,2*my_z);
					pw_dl_rdptr(&current_pointer_to_dl);
					pw_dl_init(NULL,0,NULL,0);
				}
			...
			case 22 : /*first word of short relative vector*/
			{
				    ...
				    (which_insert_mode == TO_LIST_ONLY))
				{
					...
					{
						draw3r(2*an_element.arg3,
							2*an_element.arg4,
							0);
					}
					else
						move3r(2*an_element.arg3,
							2*an_element.arg4,
							0);
