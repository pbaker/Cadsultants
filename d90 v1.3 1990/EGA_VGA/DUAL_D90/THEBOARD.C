...
char far graphics_board = EGA_BOARD;
...
{
/*make and save my little curser  (only done before calling ICU() ) */
...
int the_char;
{
	...
	if(in_listing_mode == FALSE)
	{
		if((my_x < 100) && (my_y >= 3000))
		{
			...
			_getimage(0,0,639,349,&image_save[0]);
			_clearscreen(_GCLEARSCREEN);
			_setvideomode(_TEXTC80);
...
int x,y;
{
	...
	if(graphics_board == EGA_BOARD)
		_ellipse(_GBORDER,5*x/32 - SIZE_CIRCLE,INVERT_Y - (int)(0.1139*y) -
			 (int)(SIZE_CIRCLE*0.8),
			5*x/32 + SIZE_CIRCLE,INVERT_Y - (int)(0.1139*y) + 
				(int)(SIZE_CIRCLE*0.8));
	else
