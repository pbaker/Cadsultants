...
void far get_word()
{
	if(fread(&read_element,SIZE_A_WORD,1,display_file) == 0)
		quit_redisplay = TRUE;
}
...
long temp_y,remainder,start_time,end_time,dummy;
	...
	doing_redisplay = TRUE;
			...
			quit_redisplay = FALSE;

			for( ; quit_redisplay == FALSE ; )
			{
				my_pckb();
				...
				switch(read_element.op_code)
				{
					case 1 : /*draw a line*/
					{
						my_dash_enable(read_element.font);
